...
export function createNodes(): THREE.Mesh[] {
  ...
  for (let i = 0; i < NODE_COUNT; i++) {
    ...
    const color = new THREE.Color(THREE_COLORS.TEAL).lerp(new THREE.Color(THREE_COLORS.ORANGE), t)
    ...
    const sectionIndex = Math.floor(i / (NODE_COUNT / NODE_POSITIONING.SECTION_COUNT))
    const xOffset =
      (sectionIndex - NODE_POSITIONING.CENTER_OFFSET) * NODE_POSITIONING.SECTION_SPACING

    node.position.x = xOffset + (Math.random() - 0.5) * NODE_POSITIONING.VARIANCE.X
    node.position.y = (Math.random() - 0.5) * NODE_POSITIONING.VARIANCE.Y
    node.position.z = layerConfig.zDepth + (Math.random() - 0.5) * NODE_POSITIONING.VARIANCE.Z
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (dist < CONNECTION_DISTANCE) {
        const opacity = (1 - dist / CONNECTION_DISTANCE) * CONNECTION_CONFIG.MAX_OPACITY
...
export function updateNodes(
  ...
  nodes.forEach((node, i) => {
    ...
    node.position.x = data.basePosition.x + Math.sin(time + data.phase) * floatAmplitude
    node.position.y =
      data.basePosition.y + Math.cos(time * OSCILLATION_SPEEDS.Y + data.phase) * floatAmplitude
    node.position.z =
      data.basePosition.z +
      Math.sin(time * OSCILLATION_SPEEDS.Z + data.phase) *
        (floatAmplitude * OSCILLATION_SPEEDS.Z_AMPLITUDE)

    const mouseInfluence = mousePos.x * MOUSE_CONFIG.INFLUENCE_X * layerConfig.parallax
    node.position.x += mouseInfluence
    node.position.y += mousePos.y * MOUSE_CONFIG.INFLUENCE_Y * layerConfig.parallax
    ...
    mat.opacity =
      layerConfig.opacity +
      Math.sin(time * OPACITY_CONFIG.OSCILLATION_SPEED + i * OPACITY_CONFIG.PHASE_OFFSET) *
        OPACITY_CONFIG.AMPLITUDE

    const nodeSection = Math.floor(
      (data.basePosition.x + SCALE_CONFIG.SECTION_OFFSET) / SCALE_CONFIG.SECTION_DIVISOR,
    )
